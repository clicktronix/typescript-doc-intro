...
## Остаточные параметры и аргументы (_Rest Parameters and Arguments_)

### Остаточные параметры (_Rest Parameters_)

В дополнение к использованию необязательных параметров или перегрузок для создания функций, которые могут принимать множество фиксированных значений аргументов, мы также можем определять функции, которые принимают неограниченное количество аргументов, используя остаточные параметры.

Остаточные параметры определяются после заданных параметров и используют синтаксис `...`:
...
Аннотация типа для остаточных параметров неявно задается TypeScript'ом как `any[]`. Это именно массив аргументов. Поэтому любая аннотация типа для остаточных параметров должна иметь форму `Array<T>` или `T[]` или тип кортежа (о котором мы узнаем позже).

### Остаточные аргументы

И наоборот, мы можем предоставить переменное количество аргументов для функции из массива, используя `rest` синтаксис. Например, метод массива `push` принимает любое количество аргументов:
...
Обратите внимание, что TypeScript не предполагает, что массивы неизменяемы. Это может привести к неожиданному поведению:

```TypeScript
// Выводимый тип - number[] - массив пустой или с несколькими числами,
const args = [8, 5];
const angle = Math.atan2(...args);
// Expected 2 arguments, but got 0 or more.
```

Использование остаточных аргументов может потребовать включения `--downlevelIteration`, если нам нужно поддерживать более старые среды выполнения.

Самым простым решением будет создание из массива кортежа с двумя элементами с помощью оператора `as` и типа `const`:

```TypeScript
// Теперь это является кортежем с фиксированной длиной элементов - 2
const args = [8, 5] as const;
// OK
const angle = Math.atan2(...args);
```

## Деструктуризация параметров (_Parameter Destructuring_)

Вы можете использовать деструктуризацию параметров, чтобы удобно распаковать объекты, предоставленные в качестве аргумента, в одну или несколько локальных переменных в теле функции.
...
Аннотация типа для объекта идет после синтаксиса деструктуризации:
...
Это может выглядеть немного избыточно, но здесь также можно использовать именованный тип:
...
## Присваваемость функций

### Возвращаемый тип `void`

Возвращаемый функцией тип void может привести к необычному, но ожидаемому поведению.

Если сигнатура функции возвращает тип void, это не означает, что функции не может возвращать каких-либо значений. Функция с сигнатурой `type vf = () => void` может возвращать какое то значение, но оно просто будет проигнорировано в возвращаемом типе.

Эти реализации типа `() => void` будут корректны:
...
И когда возвращаемое значение одной из этих функций присваивается другой переменной, оно сохраняет тип `void`:
...
Следующий код является корректным, даже когда `Array.prototype.push` возвращает число `number`, а метод `Array.prototype.forEach` ожидает функцию с типом возвращаемого значения `void`.
...
Есть еще один момент, о котором следует помнить: когда определение литеральной функции имеет возвращаемый тип `void`, эта функция не должна ничего возвращать.
